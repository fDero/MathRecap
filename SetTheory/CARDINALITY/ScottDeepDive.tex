...
\begin{lemma}(Trasitivity of cardinality comparison) 
    $|A| \leq |B| \land |B| \leq |C| \Rightarrow |A| \leq |C|$.
\end{lemma}
\begin{proof}
    Let $f_{AB}$ be the injection from $A$ to $B$ and $f_{BC}$ be the injection from $B$ to $C$.
    We can construct $f_{AC} = f_{BC} \circ f_{AB}$, which is an injection from $A$ to $C$, thus $|A| \leq |C|$.
\end{proof} 

\begin{lemma}(Trasitivity of mutual injection) 
    $|A| \darr |B| \land |B| \darr |C| \Rightarrow |A| \darr |C|$.
\end{lemma}
\begin{proof}
    $$ |A| \darr |B| \darr |C| \Rightarrow |A| \leq |B| \leq |C| $$
    $$ |C| \darr |B| \darr |A| \Rightarrow |C| \leq |B| \leq |A| $$
    $$ \therefore |A| \darr |C| $$ \\
\end{proof} 
